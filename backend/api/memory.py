# -*- coding: utf-8 -*-
"""
[INPUT]: 依赖 fastapi.APIRouter，依赖 memory.long_term.list_memories，依赖 memory.structured.get_user_profile，依赖 api.auth.require_api_key
[OUTPUT]: 对外提供 router (GET /memory/{user_id}，鉴权：Authorization: Bearer <api_key> 须匹配 user_id)
...
async def get_memory(user_id: str, _: str = Depends(require_api_key)) -> MemoryView:
    ...
    items = [
        MemoryItem(
            ...
            memory_type=m["metadata"].get("memory_type", "semantic"),
        )
        for m in raw
    ]
